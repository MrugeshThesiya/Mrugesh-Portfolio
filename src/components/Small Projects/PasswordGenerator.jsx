...
function PasswordGenerator() {
  ...
  return (
    <div className="flex items-center justify-center flex-col">
      ...
      <form
        className="bg-tertiary flex flex-col items-center justify-center p-[1.5rem] rounded-2xl"
        onClick={(e) => {
          e.preventDefault();
        }}
      >
      ...
      <p className="text-left mx-[2rem] mt-4 text-[1.1rem] w-[90%] md:w-[60%]">{smallProjects[1].desc}</p>
    {isToast ? <div className="rounded-md border-l-4 border-green-500 bg-green-100 p-4 fixed top-[5rem] right-[1rem]">
        <div className="flex items-center justify-between space-x-4">
          ...
          <div>
            <X onClick={() => setIsToast(false)} className="h-6 w-6 cursor-pointer text-green-600" />
          </div>
        </div>
      </div> : <></>}
    </div>
